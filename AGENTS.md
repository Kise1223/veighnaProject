...
- Current status: `M0-M9` is complete; `M7` stops at one-shot paper execution and local ledger, `M8` stops at bar-driven paper-only shadow execution plus final ledger snapshots, and `M9` extends that shadow path to deterministic tick-replay paper execution.
...
- Build the smallest closed loop first; do not plan `M10` ahead unless asked.
