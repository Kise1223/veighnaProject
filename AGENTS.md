...
- Current status: `M0-M16` is complete; `M7` stops at one-shot paper execution and local ledger, `M8` stops at bar-driven paper-only shadow execution plus final ledger snapshots, `M9` extends that shadow path to deterministic tick-replay paper execution, `M10` adds deterministic L1 top-of-book constrained partial fills plus simple `DAY/IOC` semantics while remaining paper-only, `M11` adds file-first execution analytics / TCA for `M7-M10` artifacts without changing execution boundaries, `M12` adds file-first portfolio / risk analytics plus planned-vs-executed drift over existing `M6-M11` artifacts, `M13` adds file-first benchmark / attribution analytics on top of existing `M12` portfolio analytics without changing any execution boundary, `M14` adds file-first walk-forward campaign orchestration plus campaign time-series analytics over existing `M5-M13` artifacts without changing any execution boundary, `M15` adds file-first rolling retrain model scheduling on top of `M14` campaigns without changing any execution boundary, and `M16` adds file-first schedule realism plus no-lookahead audit on top of `M15` without changing any execution boundary.
...
- Build the smallest closed loop first; do not plan `M17` ahead unless asked.
