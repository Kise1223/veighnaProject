"""Run M13 benchmark-relative analytics for one paper or shadow source."""
...
from libs.analytics.benchmark_attribution import run_benchmark_analytics
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    if args.benchmark_run_id is None and args.benchmark_source_type is None:
        parser.error("--benchmark-run-id or --benchmark-source-type is required")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_benchmark_analytics(
            project_root=ROOT,
