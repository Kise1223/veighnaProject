...
def test_execution_compare_supports_bars_ticks_full_partial_and_day_ioc(tmp_path: Path) -> None:
    workspace, ids = prepare_m11_workspace(tmp_path)

    bars_vs_ticks = compare_execution_runs(
        project_root=workspace,
        left_shadow_run_id=ids["bars_shadow_run_id"],
        right_shadow_run_id=ids["ticks_crossing_run_id"],
        compare_basis="bars_vs_ticks",
    )
    full_vs_partial = compare_execution_runs(
        project_root=workspace,
    ...
    day_vs_ioc = compare_execution_runs(
        project_root=workspace,
    ...
    day_vs_ioc_summary = store.load_compare_summary(
        compare_run_id=str(day_vs_ioc["compare_run_id"])
    )

    assert bars_vs_ticks["row_count"] >= 5
    assert full_vs_partial["row_count"] >= 5
    assert day_vs_ioc_summary.compare_basis.value == "day_vs_ioc"
    assert day_vs_ioc_summary.comparable_order_count == 1
    assert day_vs_ioc_summary.summary_json["left_only_count"] == 0
    assert day_vs_ioc_summary.summary_json["right_only_count"] == 0


def test_execution_compare_records_unmatched_instruments(tmp_path: Path) -> None:
    ...
    summary = ExecutionAnalyticsArtifactStore(workspace).load_compare_summary(
        compare_run_id=str(compare["compare_run_id"])
    )

    assert summary.comparable_order_count == 0
